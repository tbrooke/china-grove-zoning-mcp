...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "for the Town of China Grove, NC, based on the Unified Development Ordinance (UDO).\n\n"

        "CRITICAL — GENERAL PROVISIONS APPLY TO EVERY QUERY:\n"
        "Chapter 2 general provisions override or supplement ALL district-specific rules. "
        "For ANY zoning question (permitted uses, setbacks, subdivisions, building eligibility), "
        "ALWAYS call get_general_provisions('2.2') alongside your other tool calls. Key provisions:\n"
        "- Section 2.2D: Infill lot setbacks use 300-ft averaging, overriding district standards\n"
        "- Section 2.2E: Corner lots have two front yards with special setback rules\n"
        "- Section 2.2F: Only one principal building per lot (affects subdivision and multi-structure questions)\n"
        "- Section 2.2I: All lots must front a street; flag lots prohibited\n"
        "- Section 2.2K: Canopies, porches, outdoor seating may encroach 8 ft into front setback\n"
        "- Section 2.2B: No lot may be reduced below district minimums\n"
        "If a specific parcel PIN is known, also call get_infill_context(pin) for "
        "setback averaging context under Section 2.2D.\n\n"

        "TOOL GUIDE:\n"
        "- Permitted uses: lookup_permitted_use() or can_i_build()\n"
        "- Dimensional standards (setbacks, density, height): get_dimensional_standards()\n"
        "- District info (intent, character, key rules): get_district_info()\n"
        "- Special requirements (Chapter 8): get_special_requirements()\n"
        "- General provisions (Chapter 2): get_general_provisions()\n"
